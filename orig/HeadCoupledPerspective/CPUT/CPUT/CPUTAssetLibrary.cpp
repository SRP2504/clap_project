...
//-----------------------------------------------------------------------------
void CPUTAssetLibrary::RebindTextures()
{
    CPUTAssetListEntry *pMaterial = mpMaterialList;
    while( pMaterial )
    {
        ((CPUTMaterial*)pMaterial->pData)->RebindTextures();
...
void *CPUTAssetLibrary::FindAsset(const cString &name, CPUTAssetListEntry *pList, bool nameIsFullPathAndFilename)
{
    ...
    while(NULL!=pList)
    {
        if( 0 == _wcsicmp( absolutePathAndFilename.data(), pList->name.data() ) )
        {
...
CPUTMaterial *CPUTAssetLibrary::GetMaterial(const cString &name, bool nameIsFullPathAndFilename, const cString &modelSuffix, const cString &meshSuffix)
{
    ...
    DBG_UNREFERENCED_LOCAL_VARIABLE(pUniqueMaterial);

    return CPUTMaterial::CreateMaterial( absolutePathAndFilename, modelSuffix, meshSuffix );
}
