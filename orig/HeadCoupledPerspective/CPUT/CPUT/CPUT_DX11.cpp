...
// initialize the CPUT system, find the resource path automatically
//-----------------------------------------------------------------------------
CPUTResult CPUT_DX11::CPUTInitialize()
{
    CPUTOSServices *pServices = CPUTOSServices::GetOSServices();
    if (!pServices) return CPUT_ERROR_NOT_FOUND; 

    // Search a up the dir tree for the CPUT resources, fail if we can't find them
    cString exeDir, rscDir, upDir;
    rscDir.clear();
    upDir.clear();
    if (CPUTFAILED(pServices->GetExecutableDirectory(&exeDir))) return CPUT_ERROR_NOT_FOUND;

    for (int upLevel = 0; upLevel < 4; upLevel++)   // try looking up to 3 levels above current working dir
    {
        rscDir = exeDir + upDir + _L("CPUT\\resources\\");
        if (CPUTSUCCESS(pServices->DoesDirectoryExist(rscDir)))
        {
            return SetCPUTResourceDirectory(rscDir);    // Found it - set dir and return success
        }

        upDir.append(_L("..\\"));
    }

    // Did not find it - return failure code
...
bool CPUT_DX11::TestContextForRequiredFeatures()
{
    ...
    // Latest DX10.1 driver for “Intel® microarchitecture code name Sandy Bridge” doesn't support this flag,
    // but HCP doesn't make use of it anyway - so just skip this check
...
CPUTResult CPUT_DX11::CreateContext()
{
    ...
    CPUTSetDebugName( mpRenderTargetView, _L("BackBufferView") );
...
CPUTResult CPUT_DX11::CreateAndBindDepthBuffer(int width, int height)
{
    ...
    CPUTRenderTargetColor::SetActiveRenderTargetView( mpRenderTargetView );
...
void CPUT_DX11::ResizeWindow(UINT width, UINT height)
{
    ...
    mpSwapChain->ResizeBuffers(mSwapChainBufferCount, windowWidth, windowHeight, mSwapChainFormat, 0);

    // re-create the render-target view
    ID3D11Texture2D *pSwapChainBuffer = NULL;
    if(FAILED(mpSwapChain->GetBuffer( 0, __uuidof( ID3D11Texture2D), (LPVOID*) (&pSwapChainBuffer))))
    {
    ...
    if(FAILED(mpD3dDevice->CreateRenderTargetView( pSwapChainBuffer, NULL, &mpRenderTargetView)))
    {
        // resize failed - likely driver ran out of memory
        ASSERT(0,_L(""));
    }
