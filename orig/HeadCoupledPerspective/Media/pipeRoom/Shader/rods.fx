...
// -------------------------------------
    ...
    Texture2D texture_LM : register( t1 );
    ...
    texture2D texture_LM < string Name = "texture_LM"; string UIName = "texture_LM"; string ResourceType = "2D";>;
    sampler2D SAMPLER1 = sampler_state{ texture = (texture_LM);};
...
// -------------------------------------
float4 DIFFUSETEX( PS_INPUT input )
{
    return 
#ifdef _CPUT
texture_DM.Sample( SAMPLER0, (((input.UV0)) *(float2(15, 10).xyyy)) )
#else
tex2D( SAMPLER0, (((input.UV0)) *(float2(15, 10).xyyy)) )
#endif
;
}

// -------------------------------------
float4 SPECULARTEX( PS_INPUT input )
{
    return 
#ifdef _CPUT
texture_SM.Sample( SAMPLER0, (((input.UV0)) *(float2(15, 10).xyyy)) )
#else
tex2D( SAMPLER3, (((input.UV0)) *(float2(15, 10).xyyy)) )
#endif
;
}
...
float4 NORMAL( PS_INPUT input )
{
    return ((
#ifdef _CPUT
texture_NM.Sample( SAMPLER0, (((input.UV0)) *(float2(15, 10).xyyy)) )
#else
tex2D( SAMPLER2, (((input.UV0)) *(float2(15, 10).xyyy)) )
#endif
) *(2)) -(1);
}
...
{
    return (DIFFUSETEX(input)) *(10);
}

// -------------------------------------
float4 SPECULAR( PS_INPUT input )
{
    return (SPECULARTMP(input)) *(SPECULARTEX(input));
}

// -------------------------------------
float4 AMBIENTTEX( PS_INPUT input )
{
    return ( (
#ifdef _CPUT
texture_LM.Sample( SAMPLER0, (((input.UV1)) *(1)) )
#else
tex2D( SAMPLER1, (((input.UV1)) *(1)) )
#endif
 ).r );
}
...
{
    return ((DIFFUSETEX(input)) *(AMBIENTTEX(input))) *(20);
}

// -------------------------------------
float4 DIFFUSE( PS_INPUT input )
{
    return ((DIFFUSETEX(input)) *(DIFFUSETEX(input))) *(20);
}
