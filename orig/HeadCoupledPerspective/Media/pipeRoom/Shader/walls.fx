...
float4 SPECULAR( PS_INPUT input )
{
    return 
#ifdef _CPUT
texture_SM.Sample( SAMPLER0, (((input.UV0)) *(2)) )
#else
tex2D( SAMPLER3, (((input.UV0)) *(2)) )
#endif
;
}
...
{
    return ((DIFFUSETEX(input)) *(AMBIENTTEX(input))) *(5);
}

// -------------------------------------
float4 DIFFUSE( PS_INPUT input )
{
    return ((DIFFUSETEX(input)) *(DIFFUSETEX(input))) *(5);
}
