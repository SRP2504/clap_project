...
//handy defines
//-----------------------------------------------------------------------------
#define SAFE_RELEASE(p)     {if((p)){HEAPCHECK; (p)->Release(); (p)=NULL; HEAPCHECK;} }
...
public:
    CPUT() : mpCamera(NULL), mpShadowCamera(NULL), mAmbientColor(0.2f, 0.2f, 0.2f),mLightColor(1.0f, 1.0f, 1.0f) {}
    virtual ~CPUT() {}
