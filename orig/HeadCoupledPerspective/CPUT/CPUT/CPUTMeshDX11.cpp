...
CPUTResult CPUTMeshDX11::CreateNativeResources(
    ...
    PATCH - for DX10.1 compatibility
    HCP sample does not use structured buffers (DX11 specific feature), so remove this
    buffer creation to allow compatibility with previous DX versions
    ...
    // HCP doesn't need this flag, and removing it allows compatibility with some
    // DX10.1 hardware.
